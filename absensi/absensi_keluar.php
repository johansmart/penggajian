...
if(isset($_POST['btnMasuk'])){
	...
	if (count($pesanError)>=1 ){
		...
		if(mysqli_num_rows($cekQry) >= 1) {
			# JIKA SUDAH ADA (Sudah Absen)

			// Membaca data Kode pegawai
			$cekData	= mysqli_fetch_array($cekQry);
			$kodePegawai= $cekData['kd_pegawai'];
			
			// Jika Belum Absen, maka datanya baru diinput
			$jam		= date("H:i:s");
			$mySql  	= "UPDATE absensi SET jam_keluar ='$jam' WHERE kd_pegawai='$kodePegawai'";
			mysqli_query($koneksidb, $mySql) or die ("Gagal query".mysqli_errno());
			
			// Refresh
			echo "<meta http-equiv='refresh' content='2; url=absensi_keluar.php'>";
...
<title> :: Absensi Keluar ( OUT )</title> 
...
<form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" name="form1" target="_self">
  <table width="100%" class="table-list" border="0" cellspacing="1" cellpadding="4">
    <tr>
      <td colspan="3" bgcolor="#CCCCCC"><strong>ABSENSI KELUAR (OUT)</strong></td>
    </tr>
    <tr>
